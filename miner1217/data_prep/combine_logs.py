...
# extract the job logs with a runtime > 300s, and process count > 1, and store these logs' information into a new table (meaningful_job.log).
...
fs_dict = {}
for record in stat_table:
    elem_lst = mds_dict[record["JobID"]]
    if fs_dict.get(record["JobID"], -1) == -1:
        fs_dict[record["JobID"]] = []
    for elem in elem_lst:
        if elem[0] == record["start_time"] and elem[1] == record["end_time"]:
            record["mdsCPUAve"] = elem[2]
            fs_dict[record["JobID"]].append((elem[0], elem[1], record["mdsCPUAve"], record["ossCPUAve"], record["ostIOAve"]))
