...
N=15
# Extract all the performance contributing factors for each job of a given application, and store them under the directory contri_factors. These output are used by app_analyze.py for generating the data source of this application's parallel coordinate plot. hardcoded parameters: root_dir is the top directory of iominer. app_bw_ext.log and app_cnt.log are the results generated by heatmap.py 
...
# record the paths of all the logs of the top N CPU cycle consumer applications in app_path_dict. E.g., app_path_dict[app_name][0] for app_name
...
for key,value in sorted_dict.items():
    if key == app_name:
        ...
        for i in range(0, shard_cnt):
            ...
            p.apply_async(save_mul_files, [cpy_values, tmp_key, root_dir+"contri_factors"])
            if i < shard_cnt:
                start = end + 1
p.close()
p.join()
