...
def get_utilization_dist(slurm_table):
    ...
    print "node utilization dist:tot_cnt:%d, knl_cnt:%d, avail_knl_cnt:%d, over_knl_cnt:%d, has_cnt:%d, avail_has_cnt:%d, over_has_cnt:%d\n"%(tot_cnt, knl_cnt, avail_knl_cnt, overscribed_knl_cnt, has_cnt, avail_has_cnt, overscribed_has_cnt)
...
print "after retrieving slurm files\n"
(sorted_has_lst, sorted_knl_lst) = get_utilization_dist(slurm_table)
plot_path = miner_param["dataset_path"]+"slurm_has.pkl" 
save_fd = open(plot_path, 'wb')
pickle.dump(sorted_has_lst, save_fd, -1)
save_fd.close()

plot_path = miner_param["dataset_path"]+"slurm_knl.pkl" 
save_fd = open(plot_path, 'wb')
pickle.dump(sorted_knl_lst, save_fd, -1)
save_fd.close()

plotUtilizationPercent(sorted_has_lst, "has", 10)
plotUtilizationPercent(sorted_knl_lst, "knl", 10)
